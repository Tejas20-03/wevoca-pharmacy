...
import { getFaqsOrder } from '@/services/faqs-data/services';
...
const FaqOrders: React.FC = () => {
    const [faqOrders, setFaqOrders] = useState<GetFaqs_About_ResponseDataType[]>([]);
    ...
    const faqFunc = async () => {
        try {
            const response = await getFaqsOrder({ token: '' });

            if (response) {
                setFaqOrders(response);
                setLoading(false);
            } else {
                faqFunc();
            }
    ...
    return (
        <div className="grid_items">
            <BoxTitle boxTitle='Orders' />
            <div>
                {faqOrders?.length > 0 && faqOrders.map((data, index) => (
