...
import { getFaqsMedicines } from '@/services/faqs-data/services';
...
const FaqMedicines: React.FC = () => {
    const [faqMedicines, setFaqMedicines] = useState<GetFaqs_About_ResponseDataType[]>([]);
    ...
    const faqFunc = async () => {
        try {
            const response = await getFaqsMedicines({ token: '' });

            if (response) {
                setFaqMedicines(response);
                setLoading(false);
            } else {
                faqFunc();
            }
    ...
    return (
        <div className="grid_items">
            <BoxTitle boxTitle='Medicines' />
            <div>
                {faqMedicines?.length > 0 && faqMedicines.map((data, index) => (
