...
import { getFaqsPayment } from '@/services/faqs-data/services';
...
const FaqPayment: React.FC = () => {
    const [faqPayment, setFaqPayment] = useState<GetFaqs_About_ResponseDataType[]>([]);
    ...
    const faqFunc = async () => {
        try {
            const response = await getFaqsPayment({ token: '' });

            if (response) {
                setFaqPayment(response);
                setLoading(false);
            } else {
                faqFunc();
            }
    ...
    return (
        <div className="grid_items">
            <BoxTitle boxTitle='Payment Information' />
            <div>
                {faqPayment?.length > 0 && faqPayment.map((data, index) => (
