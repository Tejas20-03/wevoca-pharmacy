...
const AccordionItem: React.FC<IProps> = ({ item }) => {
  ...
  const accordionClass = hasSubMenu
    ? "cateogries-menu"
    : "cateogries-menu no-submenu";

  return (
    <Accordion
      className={accordionClass}
      expanded={expanded}
      onChange={hasSubMenu ? toggleAccordion : unableToToggleAccordion}
    >
      <AccordionSummary className="cateogries-itemLink">
        ...
        <Button onClick={toggleAccordion}>
          {hasSubMenu && <DownArrow color={"--primary-color-darker"} />}
        </Button>
      </AccordionSummary>
      <AccordionDetails>
